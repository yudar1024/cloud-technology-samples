...
//				.keyBy(0)
//				.sum(1);
		SingleOutputStreamOperator<Tuple2<String, Integer>> tuple2SingleOutputStreamOperator = text.flatMap((String line, Collector<Tuple2<String, Integer>> out) -> {
			Arrays.stream(line.split("\\W+")).forEach(x -> out.collect(Tuple2.of(x, 1)));
		}).keyBy(0).sum(1);
